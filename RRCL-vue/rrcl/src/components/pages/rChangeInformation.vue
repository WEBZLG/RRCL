            ...
            <Tabs>
                <TabPane label="非认证信息更改" icon="ios-people">
                    ...
                    <div class="btns">
                        ...
                        <Button type="primary" @click="handleSubmitF('formValidateF')" :disabled='btnDis'>提交</Button>
                    </div>
                </TabPane>
                <TabPane label="认证信息更改" icon="ios-person">
                    <Form ref="formValidateR" :model="formValidateR" label-position="left" :label-width="100" :rules="ruleValidate">
                        <FormItem label="姓名" prop="realname">
                            <Input v-model="formValidateR.realname" readonly></Input>
                        </FormItem>
                        <FormItem label="身份证号" prop="idcard">
                            <Input v-model="formValidateR.idcard" readonly></Input>
                        </FormItem>
                        <FormItem label=" " >
                            <div>
                                <Upload
                        ...
                        <FormItem label="公司名称" prop="company">
                            <Input v-model="formValidateR.company" readonly></Input>
                        </FormItem>
                        <FormItem label="">
                            <div>
                                <Upload
                        ...
                        <FormItem label="选择已有公司" prop="companyId">
                            <Select v-model="formValidateR.companyId" placeholder="请选择您所属的公司" readonly>
                                <Option  v-for="item in companyList" :key="item.id" :value="item.id">{{item.name}}</Option>
                            </Select>
                        </FormItem>
                        <FormItem label="用户级别" prop="level">
                            <RadioGroup v-model="formValidateR.level" readonly>
                                <Radio label="2">企业管理员</Radio>
                                <Radio label="5">企业员工</Radio>
                                <Radio label="6">普通用户</Radio>
                            </RadioGroup>
                        </FormItem>
                    </Form>
                    <div class="btns">
                        <Button type="success" @click="handleReset('formValidateR')">重置</Button>
                        <Button type="primary" @click="handleSubmitR('formValidateR')" :disabled='btnDis'>提交</Button>
                    </div>
                </TabPane>
            </Tabs>
        </div>
        ...
        methods: {
            ...
            handleSubmitF (name) {
                ...
                this.$refs[name].validate((valid) => {
                    ...
                    console.log(that.formValidateF.email)
                    console.log(that.formValidateF.call)
                    console.log(that.formValidateF.Address)
                    console.log(that.formValidateF.birthdate)
                    console.log(that.formValidateF.sex)
                    this.$axios.post(that.imgsrc+'/rock/user/modifyUserInfo.action',param,{
                            xhrFields: {
            ...
            getChangeInformation(){
                var that = this
                this.$axios.get(this.imgsrc+'/rock/auth/getAuthUserInfo.action',{},{
                    xhrFields: {
                        withCredentials: true
                    }          
                })
